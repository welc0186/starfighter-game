...
class ColliderComponent:
    ...
    def from_surface(cls, surface: pygame.Surface,
        ...
        return cls(surface.get_width(), surface.get_height(),
                  offset_x, offset_y, tags, ignore_tags, on_collision, mask)
    ...
    def collides_with(self, other: 'ColliderComponent', 
                     pixel_perfect: bool = False) -> bool:
...
class CollisionEvent:
    ...
    def __init__(self, entity_a: int, entity_b: int, 
            ...
            for j in range(i + 1, len(entity_list)):
                ...
                if collider_a.collides_with(collider_b, self.pixel_perfect):
                    ...
                    if self.pixel_perfect and collider_a.mask and collider_b.mask:
                        offset = (collider_b.rect.x - collider_a.rect.x, 
                                 collider_b.rect.y - collider_a.rect.y)
                        overlap_point = collider_a.mask.overlap(collider_b.mask, offset)
                    
                    event = CollisionEvent(entity_a, entity_b, collider_a, 
                                         collider_b, overlap_point)
